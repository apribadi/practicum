...
magic p = 
    let m    = spec p
        orig = [m ! k | k <- [1..p-1]]
        pats = map (pattern p m) [1..p-1]
        good = all (\pat -> pat == orig || pat == complement orig) pats
    in 
        if good then Just orig else Nothing
...
series' p a 0 acc = []
series' p a n acc = q : series' p a (n-1) q
    where q = (acc * a) `mod` p
...
pattern :: Int -> Array Int Int -> Int -> [Int]
pattern p m d = [m ! ((k * d) `mod` p) | k <- [1..p-1]]

spec :: Int -> Array Int Int
spec p = 
    let a = primitive p
    in Array.array (1, p-1) [(pow p a k, k `mod` 2) | k <- [0..p-2]]
